...
public class InteractiveTask implements Runnable {
	private StackTopApplication app;
	private Console console;
	private PrintStream tty;
	private String inputWhenWaitForEnter;

	public InteractiveTask(StackTopApplication app) {
		this.app = app;
		tty = System.err;
	...
	private void handleCommand(String command) throws Exception {
		if ("s".equals(command) || command.startsWith("s ")) {
			...
			changeDisplayMode();
		} else if ("i".equals(command) || command.startsWith("i ")) {
			changeInterval(command);
		} else if ("l".equals(command) || command.startsWith("l ")) {
			changeThreadLimit(command);
		} else if ("f".equals(command)) {
			changeThreadFilter();
		} else if ("q".equals(command) || "quit".equals(command) || "exit".equals(command)) {
			app.exit();
