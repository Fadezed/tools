...
import com.fingard.tools.stacktop.pojo.VmDetailView.ContentMode;
import com.fingard.tools.stacktop.pojo.VmDetailView.OutputFormat;
...
public class OptionAdvanceParser {
	...
	public static String parsePid(OptionParser parser, OptionSet optionSet) {
		...
		if (pid == null) {
			...
			StackTopApplication.printHelper(parser);
			System.exit(0);
		}
	...
	public static ContentMode parseContentMode(OptionSet optionSet) {
		...
		if (optionSet.hasArgument("content")) {
			...
			if ("jvm".equals(format)) {
				contentMode = ContentMode.jvm;
			} else if ("thread".equals(format)) {
